...
- uGUI: `UnityMCPServer/Assets/Scenes/MCP_UI_UGUI_TestScene.unity`（Edit Mode から配置が確認できる）
- UI Toolkit: `UnityMCPServer/Assets/Scenes/MCP_UI_UITK_TestScene.unity`（UIDocument + UXML、Edit Mode から配置が確認できる）
- IMGUI: `UnityMCPServer/Assets/Scenes/MCP_UI_IMGUI_TestScene.unity`（OnGUI のテストパネル、Edit Mode から配置が確認できる）
- uGUI/UI Toolkit/IMGUI 同居: `UnityMCPServer/Assets/Scenes/MCP_UI_AllSystems_TestScene.unity`
  - 注: シーン上に静的配置はせず、`McpAllUiSystemsTestBootstrap` が **Play Mode** 開始時に uGUI/UI Toolkit/IMGUI を生成する（Edit Mode では見えない）
    - 実装: `UnityMCPServer/Assets/Scripts/McpUiTest/McpAllUiSystemsTestBootstrap.cs`
- MCP経由のE2Eテスト（stdio / tools/call）: `mcp-server/tests/e2e/ui-automation-mcp-protocol.test.js`
- UnityConnection直結のE2Eテスト（ツールハンドラ直呼び）: `mcp-server/tests/e2e/ui-automation-scenes.test.js`
- 代表的な要素例:
...
### 実装ファイル
- `mcp-server/src/handlers/ui/UIFindElementsToolHandler.js`
- `mcp-server/src/handlers/ui/UIClickElementToolHandler.js`
- `mcp-server/src/handlers/ui/UISetElementValueToolHandler.js`
- `mcp-server/src/handlers/ui/UIGetElementStateToolHandler.js`
- `mcp-server/src/handlers/ui/UISimulateInputToolHandler.js`
- `UnityMCPServer/Packages/unity-mcp-server/Editor/Handlers/UIInteractionHandler.cs`
