...
Unity 6.3 以降で `com.akiojin.unity-mcp-server` を OpenUPM から導入したとき、Package Manager の署名ステータスが「署名なし（警告）」にならず、出所と改ざん検知の観点で安心して利用したい。
...
1. **前提** Unity 6.3+ の新規プロジェクト、**実行** OpenUPM から `com.akiojin.unity-mcp-server@X.Y.Z` をインストール、**結果** 署名ステータスが警告（署名なし/無効）にならない
...
- **FR-001**: リリースタグ `vX.Y.Z` をトリガーに、Unity UPM パッケージ（`com.akiojin.unity-mcp-server`）の署名付き tarball（`.tgz`）を生成できる必要がある
...
- **SC-001**: Unity 6.3+ で OpenUPM から導入した `com.akiojin.unity-mcp-server` が署名なし警告（黄色）にならない
