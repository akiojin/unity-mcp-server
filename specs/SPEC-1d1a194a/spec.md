...
**作成日**: 2025-11-18
**ステータス**: 下書き
**入力**: Claude CodeなどMCPクライアントでunity-mcp-serverのツールが「Capabilities: none」と表示される問題、およびツール数上限（100件）クライアントで接続できない問題を修正する

## ユーザーシナリオ＆テスト *(必須)*

### ユーザーストーリー1 - ツール一覧が正常に認識される (優先度: P1)

MCPクライアント（Claude Code等）でunity-mcp-serverに接続したとき、107個のツールがすべて正常に認識され、ユーザーがUnityエディタの操作を即座に開始できる。
...
MCPクライアントでunity-mcp-serverに接続したとき、コンソールエラーやワーニングが発生せず、クリーンな接続状態が維持される。
...
1. **前提** unity-mcp-serverを起動、**実行** MCPクライアントで接続、**結果** サーバーログにエラー/ワーニングが出力されない
2. **前提** MCPクライアントで接続済み、**実行** ツール一覧を表示、**結果** クライアントのコンソールにエラー/ワーニングが表示されない
3. **前提** Unity Editorが起動していない状態、**実行** unity-mcp-serverに接続、**結果** 接続リトライのログのみ表示され、致命的エラーは発生しない
...
ツール数上限（100件など）を持つMCPクライアントでも、利用者がカテゴリ単位で公開ツールを絞り込むことで、unity-mcp-serverを有効化して運用できる。
...
- Unity Editorが起動していない状態でunity-mcp-serverに接続した場合、capability認識に影響するか?
...
- npm package配布のため、`mcp-server/`ディレクトリ内の変更のみ対象
