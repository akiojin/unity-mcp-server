...
- [ ] **T001** [P] `mcp-server/tests/contract/`ディレクトリ作成
- [ ] **T002** [P] `mcp-server/tests/integration/`ディレクトリ作成（既存の場合はスキップ）
- [ ] **T003** [P] `mcp-server/tests/unit/core/`ディレクトリ作成（既存の場合はスキップ）
...
- [ ] **T004** [P] `mcp-server/tests/contract/mcp-capabilities.test.js`作成
  ...
  - **ファイルパス**: `mcp-server/tests/contract/mcp-capabilities.test.js`
  - **依存関係**: なし

- [ ] **T005** [P] `mcp-server/tests/contract/mcp-handler-registration.test.js`作成
  ...
  - **期待される結果**: ❌ REDフェーズ（テスト失敗）
  - **ファイルパス**: `mcp-server/tests/contract/mcp-handler-registration.test.js`
  - **依存関係**: なし
...
- [ ] **T006** `mcp-server/tests/integration/mcp-tools-list.test.js`作成
...
- [ ] **T007** `mcp-server/tests/unit/core/server-capabilities.test.js`作成
  ...
  - **期待される結果**: ❌ REDフェーズ（テスト失敗）
  - **ファイルパス**: `mcp-server/tests/unit/core/server-capabilities.test.js`
  - **依存関係**: T004, T005完了後

**テスト実行（REDフェーズ確認）**:

- [ ] **T008** テストを実行してREDフェーズを確認
  - **コマンド**: `cd mcp-server && npm run test:ci`
  - **期待される結果**: T004, T005, T007のテストが失敗（❌ RED）
  - **コミット**: `test(mcp-server): add capabilities contract tests (RED phase)`
...
- [ ] **T009** `mcp-server/src/core/server.js`のcapabilities宣言を修正（1箇所目）
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 29-36付近)
  - **変更内容**:
    ```diff
    capabilities: {
...
- [ ] **T010** `mcp-server/src/core/server.js`のcapabilities宣言を修正（2箇所目: createServer関数）
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 300-305付近)
  - **変更内容**:
    ```diff
    capabilities: {
      tools: { listChanged: true },
...
- [ ] **T011** `mcp-server/src/core/server.js`からListResourcesRequestSchemaハンドラー削除
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 66-71付近)
  - **削除対象**:
    ```javascript
...
- [ ] **T012** `mcp-server/src/core/server.js`からListPromptsRequestSchemaハンドラー削除
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 73-78付近)
  - **削除対象**:
    ```javascript
...
- [ ] **T013** `mcp-server/src/core/server.js`からListResourcesRequestSchemaハンドラー削除（createServer関数内）
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 314-316付近)
  - **削除対象**:
    ```javascript
...
- [ ] **T014** `mcp-server/src/core/server.js`からListPromptsRequestSchemaハンドラー削除（createServer関数内）
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 318-320付近)
  - **削除対象**:
    ```javascript
...
- [ ] **T015** `mcp-server/src/core/server.js`からListResourcesRequestSchema, ListPromptsRequestSchemaのimport削除
  - **ファイルパス**: `mcp-server/src/core/server.js` (Line 3-8付近)
  - **変更内容**:
    ```diff
    import {
...
- [ ] **T016** テストを実行してGREENフェーズを確認
  ...
  - **コミット**: `fix(mcp-server): remove empty capabilities causing "Capabilities: none"`
...
- [ ] **T018** [P] `mcp-server/README.md`にトラブルシューティングセクション追加
  - **ファイルパス**: `mcp-server/README.md` (Troubleshooting セクション)
  - **追加内容**:
    ...
    - MCP client互換性チェック
  - **依存関係**: T017完了後
  - **コミット**: `docs(mcp-server): add "Capabilities: none" troubleshooting guide`

### Code Quality

- [ ] **T019** ESLint実行
  - **コマンド**: `cd mcp-server && npm run lint`
  - **期待される結果**: エラー/ワーニングなし
  - **依存関係**: T018完了後

- [ ] **T020** Prettier実行
  - **コマンド**: `cd mcp-server && npm run format`
  - **期待される結果**: フォーマット完了
  - **依存関係**: T019完了後
...
- [ ] **T022** 最終コミット作成（Conventional Commits準拠）
  - **コミットメッセージ**:
    ```
    fix(mcp-server): resolve MCP capabilities recognition issue
...
```bash
# T001, T002, T003を同時実行:
mkdir -p mcp-server/tests/contract
mkdir -p mcp-server/tests/integration
mkdir -p mcp-server/tests/unit/core
```
...
```bash
# T004, T005を同時実行:
Task 1: "mcp-server/tests/contract/mcp-capabilities.test.js作成"
Task 2: "mcp-server/tests/contract/mcp-handler-registration.test.js作成"
```
