...
test_release_creates_pr_without_auto_merge() {
  ...
  if [ -f "$REPO_ROOT/scripts/create-release-branch.sh" ]; then
    if grep -q "gh.*pr create\|release" "$REPO_ROOT/scripts/create-release-branch.sh"; then
      echo "✓ PASS: create-release-branch.shがPR作成機能を持っている"
      return 0
    else
      echo "⚠️  WARN: create-release-branch.shのPR作成機能が明示的でない"
      return 0
    fi
  else
    echo "⚠️  WARN: create-release-branch.shが存在しない（/releaseコマンドが直接実装している可能性）"
    return 0
  fi
}
