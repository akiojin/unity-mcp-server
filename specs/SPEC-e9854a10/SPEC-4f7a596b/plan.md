...
## 概要
edit_script_structuredによるメソッド全体置換へ依存した結果、1〜2行規模の修正で余計な差分や括弧崩れが頻発した。過去に行ベース編集を許容した際には括弧整合が破綻し、最終的にedit_script_structuredのみを許可する方針へ転換した経緯がある。本計画では、その制約を維持しつつ「小さな断片編集を安全に適用する」ための新ツール（仮称 `edit_script_snippet`）を設計・実装する。

## 現状課題の整理
- **括弧整合問題**: 以前の行編集は`{}`バランス崩壊を多発させ、ビルド不能に。再導入時は検証ステップが必須。
- **ツール粒度のミスマッチ**: edit_script_structuredはメソッド単位編集が前提で、LLMが求める1〜2行の条件削除には過剰。
...
2. **アンカー解決エンジン**  
   - LSPの`mcp/findText`や既存`search_script`結果を活用してアンカー候補を特定。  
...
5. **フォールバック設計**  
   - アンカーが解決できない場合や差分が80文字超の場合は早期に失敗し、edit_script_structuredへのフォールバック案内を含む。  
...
- [ ] 既存`search_script`のレスポンス構造を確認し、アンカー解像度に再利用できるか検証する。
...
- **Phase 3 (Integration & Polish)**  
  ...
  - edit_script_structuredとの併用ガイダンスをドキュメント化し、括弧崩れ防止策を強調。
...
- **既存LLMフローとの不整合**: CLAUDE.mdやREADMEの手順を更新し、edit_script_structured優先指示を調整。
