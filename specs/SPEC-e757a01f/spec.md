...
**US-8.2: 進捗確認**
- get_code_index_statusでリアルタイムの進捗情報を取得
...
- バックグラウンドビルド実行中も`get_code_index_status`が即座に進捗を返す
- バックグラウンドビルド実行中も`search_script`、`find_script_symbol`が正常に動作する
...
- get_code_index_statusが成功することを確認
...
10. **前提** 大規模プロジェクト（10000+ファイル）、**実行** build_code_index、**結果** 1秒以内にジョブIDが返り、バックグラウンドでビルド継続
11. **前提** インデックスビルドが実行中、**実行** get_code_index_status、**結果** 進捗情報（処理済み/全体、処理速度）が返る
12. **前提** インデックスビルドが実行中、**実行** 再度build_code_index、**結果** エラーと既存のジョブIDが返る
13. **前提** npmキャッシュをクリアした状態、**実行** `npx @akiojin/unity-mcp-server@latest --help`、**結果** 30秒以内に完了
14. **前提** 対応外プラットフォーム、**実行** get_code_index_status、**結果** WASMフォールバックで成功
15. **前提** indexWatcherがバックグラウンドビルドを実行中、**実行** ping、**結果** 1秒以内に応答が返る
16. **前提** indexWatcherがバックグラウンドビルドを実行中、**実行** get_code_index_status、**結果** 即座に進捗情報が返る
17. **前提** indexWatcherがバックグラウンドビルドを実行中、**実行** search_script、**結果** 検索結果が正常に返る
...
- **FR-051**: シンボル検索（find_script_symbol）はDBインデックスが構築されている場合のみ結果を返し、未構築の場合はLSPフォールバックせずエラーを返す必要がある
- **FR-052**: 参照検索（find_script_refs）はDBインデックスが構築されている場合のみ実行され、未構築の場合はエラーを返す必要がある
- **FR-053**: インデックスが構築中の場合、システムは`index_building`エラーとビルド進捗情報（処理済み/全体、パーセンテージ）を返す必要がある
- **FR-054**: インデックスが未構築で構築ジョブも実行されていない場合、システムは`index_not_ready`エラーと`build_code_index`の使用を促すヒントを返す必要がある
...
- **FR-034**: システムは編集結果のハッシュを返して edit_script_structured との併用時の重複適用を防ぐ必要がある
...
- **FR-060**: システムは既存のbuild_code_indexツールインターフェースを維持し、後方互換性を保つ必要がある
