...
**US-8.2: 進捗確認**
- code_index_statusでリアルタイムの進捗情報を取得
...
**受け入れ基準**:
- バックグラウンドビルド実行中も`system_ping`が1秒以内に応答する
- バックグラウンドビルド実行中も`code_index_status`が即座に進捗を返す
- バックグラウンドビルド実行中も`script_search`、`script_symbol_find`が正常に動作する
...
**受け入れ基準**:
- `npx @akiojin/unity-mcp-server@latest --help` が30秒以内に完了する
...
10. **前提** 大規模プロジェクト（10000+ファイル）、**実行** code_index_build、**結果** 1秒以内にジョブIDが返り、バックグラウンドでビルド継続
11. **前提** インデックスビルドが実行中、**実行** code_index_status、**結果** 進捗情報（処理済み/全体、処理速度）が返る
12. **前提** インデックスビルドが実行中、**実行** 再度code_index_build、**結果** エラーと既存のジョブIDが返る
13. **前提** npmキャッシュをクリアした状態、**実行** `npx @akiojin/unity-mcp-server@latest --help`、**結果** 30秒以内に完了
14. **前提** 対応外プラットフォーム、**実行** code_index_status、**結果** WASMフォールバックで成功
15. **前提** indexWatcherがバックグラウンドビルドを実行中、**実行** system_ping、**結果** 1秒以内に応答が返る
16. **前提** indexWatcherがバックグラウンドビルドを実行中、**実行** code_index_status、**結果** 即座に進捗情報が返る
17. **前提** indexWatcherがバックグラウンドビルドを実行中、**実行** script_search、**結果** 検索結果が正常に返る
...
- **FR-051**: シンボル検索（script_symbol_find）はDBインデックスが構築されている場合のみ結果を返し、未構築の場合はLSPフォールバックせずエラーを返す必要がある
- **FR-052**: 参照検索（script_refs_find）はDBインデックスが構築されている場合のみ実行され、未構築の場合はエラーを返す必要がある
- **FR-053**: インデックスが構築中の場合、システムは`index_building`エラーとビルド進捗情報（処理済み/全体、パーセンテージ）を返す必要がある
- **FR-054**: インデックスが未構築で構築ジョブも実行されていない場合、システムは`index_not_ready`エラーと`code_index_build`の使用を促すヒントを返す必要がある
...
- **NFR-019**: バックグラウンドビルド中も`system_ping`が1秒以内に応答する必要がある
...
- **NFR-016**: `npx @akiojin/unity-mcp-server@latest --help` が30秒以内に完了する必要がある
