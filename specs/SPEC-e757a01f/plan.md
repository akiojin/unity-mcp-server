...
既存の `edit_script_structured` はメソッド全体の置換やクラス単位の追加に最適化されており、1-2行規模の局所編集には過剰です。過去に行ベース編集を許容した際には括弧整合が破綻し、最終的に `edit_script_structured` のみを許可する方針へ転換した経緯があります。本設計では、その制約を維持しつつ「小さな断片編集を安全に適用する」ための新ツール `edit_script_snippet` を設計します。
...
- LSPの `mcp/findText` または既存 `search_script` 結果を活用してアンカー候補を特定
...
- `edit_script_structured` へのフォールバック案内を含む
...
- [ ] 既存 `search_script` のレスポンス構造確認、アンカー解像度への再利用可能性検証
