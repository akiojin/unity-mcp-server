...
- [ ] **T001** [P] Addressablesパッケージインストール確認とテスト環境セットアップ
  ...
  - UnityMCPServerプロジェクトの `Packages/manifest.json` に依存関係が記載されていることを確認
  - UnityMCPServerプロジェクトを開いてAddressables Windowが表示されることを確認
...
- [ ] **T003** [P] Node側ハンドラーディレクトリ構造作成
  - ファイル: `mcp-server/src/handlers/addressables/` (新規ディレクトリ)
  - `mcp-server/src/handlers/addressables/` ディレクトリを作成
...
- [ ] **T008** `UnityMCPServer/Packages/unity-mcp-server/Editor/Handlers/AddressablesHandler.cs` を作成（スケルトン）
  - ファイル: `UnityMCPServer/Packages/unity-mcp-server/Editor/Handlers/AddressablesHandler.cs` (新規)
...
- [ ] **T009** AddressablesHandler: add_entry, remove_entry, set_address 実装
  - ファイル: `UnityMCPServer/Packages/unity-mcp-server/Editor/Handlers/AddressablesHandler.cs` (更新)
...
- [ ] **T015** [P] `mcp-server/src/handlers/addressables/AddressablesManageToolHandler.js` 実装
  - ファイル: `mcp-server/src/handlers/addressables/AddressablesManageToolHandler.js` (新規)
  - BaseToolHandlerを継承
...
- [ ] **T016** [P] `mcp-server/src/handlers/addressables/AddressablesBuildToolHandler.js` 実装
  - ファイル: `mcp-server/src/handlers/addressables/AddressablesBuildToolHandler.js` (新規)
  - BaseToolHandlerを継承
...
- [ ] **T017** [P] `mcp-server/src/handlers/addressables/AddressablesAnalyzeToolHandler.js` 実装
  - ファイル: `mcp-server/src/handlers/addressables/AddressablesAnalyzeToolHandler.js` (新規)
  - BaseToolHandlerを継承
  - ツール名: `addressables_analyze`
  - JSON Schemaでリクエストパラメータを定義（contracts/addressables-analyze.jsonに準拠）
  - validate(), execute(), formatResponse() 実装

- [ ] **T018** `mcp-server/src/handlers/index.js` で新規ハンドラーをエクスポート
  - ファイル: `mcp-server/src/handlers/index.js` (更新)
...
- [ ] **T026** [P] ページング機能の検証とトークン最適化
  - ファイル: `mcp-server/src/handlers/addressables/*.js` (更新)
