...
|------|----------------|--------|----------|------|
| 50,000件挿入 | ~50ms | ~126ms | **74.6ms** | 41%改善 vs sql.js |
| 同一SQL×1000回 | ~5μs/回 | ~21μs/回 | **15.6μs/回** | 26%改善 vs sql.js |
| キャッシュヒット | - | - | **4.5μs/回** | prepare()再利用時 |
| LIKE検索(100K行) | ~2ms | ~5ms | **12.4ms** | CI環境での実測値 |
| ネイティブビルド | 必要 | 不要 | 不要 | - |
...
- **キャッシュヒット時**: ~4.5μs（sql.jsの`prepare()`は~75μs）
