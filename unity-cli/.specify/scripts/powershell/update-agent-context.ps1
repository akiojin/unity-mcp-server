...
function Update-ExistingAgentFile {
    ...
    $newChangeEntry = ''
    if ($techStack) { $newChangeEntry = "- ${CURRENT_BRANCH}: ${techStack} を追加" }
    elseif ($NEW_DB -and $NEW_DB -notin @('N/A','NEEDS CLARIFICATION')) { $newChangeEntry = "- ${CURRENT_BRANCH}: ${NEW_DB} を追加" }
    ...
    for ($i=0; $i -lt $lines.Count; $i++) {
        $line = $lines[$i]
        if ($line -eq '## Active Technologies' -or $line -eq '## 利用技術') {
            $output.Add($line)
            $inTech = $true
            continue
        }
        ...
        if ($line -eq '## Recent Changes' -or $line -eq '## 最近の変更') {
            $output.Add($line)
        ...
        if ($line -match '最終更新日:\s*\d{4}-\d{2}-\d{2}') {
            $output.Add(($line -replace '\d{4}-\d{2}-\d{2}',$Date.ToString('yyyy-MM-dd')))
            continue
        }
        $output.Add($line)
