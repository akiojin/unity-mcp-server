...
test_unity_mcp_tool_allowed() {
    local input='{"tool_name":"mcp__unity-mcp-server__edit_structured","tool_input":{"path":"Assets/Scripts/Foo.cs"}}'
    local exit_code
    ...
    if [ $exit_code -eq 0 ]; then
        record_test "unity-mcp-server tool allowed" "PASS"
    else
        record_test "unity-mcp-server tool allowed" "FAIL"
    fi
}
...
test_write_unity_cs_blocked() {
    local input='{"tool_name":"Write","tool_input":{"file_path":"Packages/unity-mcp-server/Runtime/NewScript.cs"}}'
    local exit_code
...
# テストケース9: UnityMCPServer/ 配下の C# ファイルはブロック
test_unity_mcp_server_dir_blocked() {
    local input='{"tool_name":"Edit","tool_input":{"file_path":"UnityMCPServer/Packages/unity-mcp-server/Editor/Handlers/ScreenshotHandler.cs"}}'
    local exit_code
    ...
    if [ $exit_code -eq 2 ]; then
        record_test "UnityMCPServer directory C# file blocked" "PASS"
    else
        record_test "UnityMCPServer directory C# file blocked" "FAIL"
    fi
}
