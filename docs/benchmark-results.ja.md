...
| ツール | 最適化 | サイズ削減 |
|--------|--------|-----------|
| `find_symbol` | ファイルごとにグループ化 | **40%** |
| `find_refs` | ファイルごとにグループ化 | **40%** |
| `search` | ファイルごとにグループ化 | **40%** |
| `get_symbols` | endLine/endColumn 削除 | **50%** |
| **全体** | null フィールド省略 | **47%** |
...
|------|------------------------|------|------------|------|------|
| シンボル検索 | `find_symbol` | **瞬時** | `grep "class "` | 353ms | **unity-mcp-server 勝利** |
| 参照検索 | `find_refs` | **瞬時** | `grep "Response"` | 346ms | **unity-mcp-server 勝利** |
| コード検索 | `search` | **瞬時** | `grep` | 346ms | **unity-mcp-server 勝利** |
| ファイル読み取り | `read` | **瞬時** | `Read` | **瞬時** | **同等**（両方瞬時） |
| ファイル一覧 | `get_index_status` | **瞬時** | `Glob` | **瞬時** | **同等**（両方瞬時） |

### コンテキスト圧縮

| ツール | 出力サイズ | 標準ツール | 出力サイズ | 圧縮率 |
|--------|-----------|------------|-----------|--------|
| `read` | 200行 (8KB) | `Read` | 358行 (13KB) | **1.6倍小さい** |
| `find_symbol` | 34シンボル (3KB) | `grep` | 209行 (15KB) | **5倍小さい** |
| `find_refs` | 20参照 (4KB) | `grep` | 全行 | **3倍小さい** |
| `search` | 7ファイル、スニペット (5KB) | `grep` | 全行 | **3倍小さい** |
...
- find_symbol: 種類/位置メタデータ付きの 34 構造化シンボル
...
| ユースケース | 推奨ツール | 理由 |
|-------------|-----------|------|
| クラス/メソッドを名前で検索 | `find_symbol` | インデックス済み、構造化結果 |
| シンボルの全使用箇所を検索 | `find_refs` | コンテキストスニペット、ページネーション |
| コードパターンを検索 | `search` | 正規表現対応、スコープフィルタリング |
| 特定ファイルを読み取り | `read` | 行制限、プロジェクトパス |
| インデックスステータス確認 | `get_index_status` | カバレッジ、ビルド進捗 |
...
| ツール | タイムアウト可能性 | 回避策 |
|--------|-------------------|--------|
| `get_symbols` | あり | 代わりに `find_symbol` を使用 |
| `find_symbol` | なし (DB ベース) | N/A |
| `find_refs` | なし (ファイルベース) | N/A |
...
build_index({ excludePackageCache: true })

// Library/PackageCache を含む（デフォルト、完全インデックス）
build_index({ excludePackageCache: false })
```
...
```javascript
// 初回ビルド（推奨）
build_index({ forceRebuild: true, excludePackageCache: true })
...
**推奨**: 新プロジェクト開始時は常に `build_index` を実行して高速シンボル操作を有効化してください。
