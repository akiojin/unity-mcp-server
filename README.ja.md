...
Unity MCP Server は以下のコンポーネントを使用しています：

| コンポーネント | 用途 | 配布方式 | サイズ |
|--------------|------|---------|-------|
| **sql.js** | コードインデックスDB | npmパッケージに同梱（WASM） | ~2.5 MB |
| **csharp-lsp** | C#シンボル解析 | 初回使用時にダウンロード | ~80 MB/プラットフォーム |

#### sql.js（v2.44.0以降）

コードインデックスDBには **sql.js**（SQLiteのWASM/JavaScript実装）を使用しています。

**選択理由**:

- **npx互換性**: ネイティブモジュールのコンパイル不要で、`npx`経由の起動が即座に完了
- **クロスプラットフォーム**: プラットフォーム固有バイナリ不要、どの環境でも動作
- **十分なパフォーマンス**: 128,000シンボルでもミリ秒単位で応答

**実測パフォーマンス**（128,061シンボル、44MB DB）:
...
> **Note**: v2.43.x以前は better-sqlite3（ネイティブCバインディング）を使用していましたが、
> `npx`経由での初回起動時にネイティブモジュールのコンパイルで30秒以上かかる問題がありました（Issue #168）。
> sql.jsへの移行により、初回起動でもタイムアウトなく即座に動作します。

#### csharp-lsp

C#シンボル解析には **csharp-lsp**（Roslyn LSP）を使用しています。

**配布方式**:

- サイズが大きい（プラットフォームあたり ~80 MB、全6プラットフォームで ~480 MB）
- npmパッケージに同梱するには大きすぎる
- スクリプト編集ツールの初回使用時にGitHub Releaseからダウンロード

#### サポートプラットフォーム

| プラットフォーム | sql.js | csharp-lsp |
|----------------|--------|------------|
| Linux x64 | ✅ WASM（全OS共通） | ✅ ダウンロード (~79 MB) |
| Linux arm64 | ✅ WASM（全OS共通） | ✅ ダウンロード (~86 MB) |
| macOS x64 | ✅ WASM（全OS共通） | ✅ ダウンロード (~80 MB) |
| macOS arm64 (Apple Silicon) | ✅ WASM（全OS共通） | ✅ ダウンロード (~86 MB) |
| Windows x64 | ✅ WASM（全OS共通） | ✅ ダウンロード (~80 MB) |
| Windows arm64 | ✅ WASM（全OS共通） | ✅ ダウンロード (~85 MB) |

#### フォールバック動作

- **sql.js**: WASMロードに失敗した場合、コードインデックス機能が無効化（明確なエラーメッセージを返却）
...
- 対応バージョン: Unity 2020.3 LTS以降 / Node.js 18.x・20.x・22.x LTS（23 以上のメジャーバージョンでは起動を拒否）
  - v2.44.0以降、ネイティブモジュール不要（sql.js使用）のため、どのNode.jsバージョンでも即座に起動します。
...
> **Note (v2.44.0以降)**: unity-mcp-serverは **sql.js**（WASMベースのSQLite）を使用しているため、ネイティブモジュールのコンパイルは不要です。`npx @akiojin/unity-mcp-server@latest` はどのプラットフォームでも即座に起動します。
...
- **ローカル開発**: リポジトリを clone した場合は `npm ci --workspace=mcp-server` を実行してから使用します。

環境別の注意点:

- **Windows / macOS / Linux**: Node.js 18.x / 20.x / 22.x LTSを使用してください。23以上のメジャーバージョンはサポート外です。
- **Docker / Linux コンテナ**: Node.js 20.x または 22.x ベースのイメージを使用してください。
