...
Unity MCP Server は大規模 Unity プロジェクトでテスト済みです：

- **C# ファイル数**: 100,000+ ファイル
- **コードインデックス**: 575,000+ シンボル
- **インデックス構築速度**: 約950ファイル/秒
- **パッケージ**: UniTask, VContainer, UniRx, Google Mobile Ads SDK, NuGetForUnity など

コードインデックス（SQLite ベース）は大規模コードベースを効率的に処理し、数十万ファイル規模でのシンボル検索・構造化編集を実現します。
