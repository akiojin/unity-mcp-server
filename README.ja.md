...
コードインデックスツールと標準Claude Codeツールの応答時間比較：

| ツール | 応答時間 | 状態 | 備考 |
|--------|----------|------|------|
| `code_index_status` | 約9秒 | 成功 | better-sqlite3が利用不可の場合は明確なエラーを返却 |
| `code_index_build` | 約8秒 | 成功 | バックグラウンドジョブを開始 |
| `code_index_update` | 約10秒 | 成功 | ファイル不在時も適切なエラーを返却 |
| `script_symbol_find` | 約107秒 | タイムアウト | Connection closed（LSPタイムアウト問題） |
| **Read** (Claude Code) | 約11秒 | 成功 | MCP非依存 |
| **Grep** (Claude Code) | 約10秒 | 成功 | MCP非依存 |

### 既知の問題: LSPタイムアウト

`script_symbol_find`などのLSPベースツールは、以下の理由で約60〜107秒後にタイムアウトする場合があります：

1. LSPリクエストタイムアウト（デフォルト60秒）がプロセス再起動をトリガー
2. ハートビート（30秒間隔）がLSPインスタンスをリセット
3. 複数のエラーハンドラが競合しMCP接続が切断

**回避策**: シンボル検索には、問題が解決されるまで標準の`Grep`ツールをフォールバックとして使用してください。
