...
import {
  ...
  createSceneHandler
} from '../../../../src/tools/scene/createScene.js';
    ...
    sendCommandSpy.mock.mockImplementation(() =>
      Promise.resolve({
        ...
        result: {
          ...
          isLoaded: true,
          summary:
            'Created and loaded scene "MyLevel" at "Assets/Levels/MyLevel.unity" (build index: 3)'
        }
      })
    );
    ...
    sendCommandSpy.mock.mockImplementation(() =>
      Promise.resolve({
        ...
        result: {
          ...
          isLoaded: false,
          summary:
            'Created scene "BackgroundScene" at "Assets/Scenes/BackgroundScene.unity" (not loaded)'
        }
      })
    );
  ...
  it('should handle Unity error response', async () => {
    sendCommandSpy.mock.mockImplementation(() =>
      Promise.resolve({
        status: 'error',
        error:
          'Scene with name "ExistingScene" already exists at path "Assets/Scenes/ExistingScene.unity"'
      })
    );
