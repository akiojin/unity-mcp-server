...
describe('ConsoleReadToolHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new ConsoleReadToolHandler(mockUnityConnection);
  });
  ...
  describe('execute', () => {
    it('should use includeStackTrace=false as default', async () => {
    ...
    it('should handle Unity response errors', async () => {
      ...
      await assert.rejects(() => handler.execute({}), /Console read failed/);
    });
