...
describe('UIFindElementsToolHandler', () => {
  let handler;
  ...
  beforeEach(() => {
    ...
    handler = new UIFindElementsToolHandler(mockUnityConnection);
  });
  ...
  describe('execute', () => {
    ...
    it('should connect if not connected', async () => {
      ...
      mockUnityConnection.sendCommand.mock.mockImplementationOnce(() =>
        Promise.resolve({ elements: [], count: 0 })
      );
    ...
    it('should handle Unity errors', async () => {
      ...
      await assert.rejects(async () => await handler.execute({}), { message: errorMessage });
    });
  });
