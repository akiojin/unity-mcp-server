...
import { UISimulateInputToolHandler } from '../../../../src/handlers/ui/UISimulateInputToolHandler.js';
import { BaseToolHandler } from '../../../../src/handlers/base/BaseToolHandler.js';

describe('UISimulateInputToolHandler', () => {
  let handler;
  ...
  beforeEach(() => {
    ...
    handler = new UISimulateInputToolHandler(mockUnityConnection);
  });
  ...
  describe('validate', () => {
    ...
    it('should validate inputSequence is array', () => {
      assert.throws(() => handler.validate({ inputSequence: 'not an array' }), {
        message: 'inputSequence must be an array'
      });
    });

    it('should validate inputSequence is not empty', () => {
      assert.throws(() => handler.validate({ inputSequence: [] }), {
  ...
  describe('execute', () => {
    it('should execute simple click sequence', async () => {
      const mockResponse = {
        success: true,
        results: [{ success: true, elementPath: '/Canvas/Button', message: 'Clicked' }],
        totalActions: 1
      };
    ...
    it('should execute complex sequence with multiple actions', async () => {
      const mockResponse = {
        success: true,
        results: [{ success: true }, { wait: 500 }, { success: true }],
        totalActions: 2
      };
      ...
      assert.deepStrictEqual(
        mockUnityConnection.sendCommand.mock.calls[0].arguments[1].waitBetween,
        500
      );
      assert.deepStrictEqual(result, mockResponse);
    });
    ...
    it('should execute without state validation when specified', async () => {
      ...
      assert.deepStrictEqual(
        mockUnityConnection.sendCommand.mock.calls[0].arguments[1].validateState,
        false
      );
    });
