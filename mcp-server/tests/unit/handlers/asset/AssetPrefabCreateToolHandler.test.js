...
import { AssetPrefabCreateToolHandler } from '../../../../src/handlers/asset/AssetPrefabCreateToolHandler.js';
import { BaseToolHandler } from '../../../../src/handlers/base/BaseToolHandler.js';

describe('AssetPrefabCreateToolHandler', () => {
  let handler;
  ...
  beforeEach(() => {
    ...
    handler = new AssetPrefabCreateToolHandler(mockUnityConnection);
  });
  ...
  describe('validate', () => {
    ...
    it('should validate prefabPath format', () => {
      ...
      assert.throws(() => handler.validate({ prefabPath: 'Assets/test.txt' }), {
        message: 'prefabPath must start with Assets/ and end with .prefab'
      });
    });
  ...
  describe('execute', () => {
    ...
    it('should overwrite existing prefab when specified', async () => {
      ...
      assert.deepStrictEqual(
        mockUnityConnection.sendCommand.mock.calls[0].arguments[1].overwrite,
        true
      );
      assert.deepStrictEqual(result, mockResponse);
    });
