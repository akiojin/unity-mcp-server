...
describe('AssetMaterialCreateToolHandler', () => {
  let handler;
  ...
  before(() => {
    ...
    handler = new AssetMaterialCreateToolHandler(mockUnityConnection);
  });
  ...
  describe('validate', () => {
    ...
    it('should validate materialPath format', () => {
      assert.throws(() => handler.validate({ materialPath: 'invalid/path' }), {
        message: 'materialPath must start with Assets/ and end with .mat'
      });

      assert.throws(() => handler.validate({ materialPath: 'Assets/test' }), {
        message: 'materialPath must start with Assets/ and end with .mat'
      });

      assert.throws(() => handler.validate({ materialPath: 'test.mat' }), {
        message: 'materialPath must start with Assets/ and end with .mat'
      });
    });
  ...
  describe('execute', () => {
    ...
    it('should handle Unity errors', async () => {
      ...
      await assert.rejects(handler.execute({ materialPath: 'Assets/Test.mat' }), {
