...
describe('Connection Performance Tests', () => {
  ...
  test('should measure connection establishment time', { skip: TestEnvironment.shouldSkipTest('unity') }, async () => {
    ...
    assert(duration < TEST_CONFIG.TIMING.CONNECTION_TIMEOUT, 
      `Connection took ${duration}ms, should be under ${TEST_CONFIG.TIMING.CONNECTION_TIMEOUT}ms`);
  });

  test('should measure command execution throughput', { skip: TestEnvironment.shouldSkipTest('unity') }, async () => {
    ...
    assert(averagePerCommand < 100, 
      `Average command execution time ${averagePerCommand}ms exceeds threshold`);
    
    console.log(`Performance: ${commandCount} commands in ${totalDuration}ms (${averagePerCommand}ms avg)`);
  });
  ...
  test('should benchmark concurrent command performance', async () => {
    ...
    const promises = Array.from({ length: concurrentCount }, (_, i) => 
      new Promise(resolve => setTimeout(resolve, 20 + (i * 5)))
    );
    ...
    assert(duration < concurrentCount * 25, 
      `Concurrent execution too slow: ${duration}ms`);
