...
- Releases are generated by release-please on `main`.
  - If changes are in develop, first run `scripts/prepare-release-pr.sh` to open the develop→main PR (auto-merge after checks).
  - Then run `scripts/create-release-pr.sh` to open the `chore(release): x.y.z` PR (auto-merge after checks).
