...
- [ ] S10-01: get_script_symbols で FinalTestClass/TestMethod11/12 確認
- [ ] S10-02: find_script_symbol で namePath 補助確定
- [ ] S10-E01: 存在しないファイルで get_script_symbols は fail
...
- 例（概念）:
  ...
  - S10-01: get_script_symbols で FinalTestClass 確認 → pending
...
- ただし、可能な限り以下のエビデンス・エスカレーションで再判定を試みる（RESULTS_FORMAT.md 参照）。
  - 差分検証: `read_script` で before/after を取り `beforeHash/afterHash/verified` を記録。期待に照らして pass/fail を再判定。
  - 構造検証: `get_script_symbols` によりシンボル有無/開始行の変化を確認。
  - 参照検証: `find_script_refs` により参照件数の期待を確認。
...
- 挿入手順: `edit_script_structured(operation=insert_after, path=Assets/Scripts/GigaTestFile.cs, symbolName=FinalTestClass, newText=\"        <定義>\\n\")`
- 利用方針:
...
2. UnityMCP ツール到達性: `get_script_symbols` など最小呼び出しが応答することを確認（0件でも可）。
3. パス制約: 以降の `path`/`relative` は必ず `Assets/` または `Packages/` 起点で指定する。
4. インデックス状況: `get_code_index_status` でカバレッジを確認し、低ければ（任意）`build_code_index` を実行して再確認する。
5. 到達性確認: 最小ファイルに対して `get_script_symbols` が成功することを確認（0件でも可）。
...
- 禁止: LSPサーバを直接起動・操作しない。必ず UnityMCP の `script_*` ツールで検証する（例: `get_script_symbols`, `edit_script_structured`）。
- パスは必ず `Assets/` または `Packages/` 起点の相対パス
- `namePath`/`symbolName` は `Outer/Nested/Member` 形式（例: `FinalTestClass/TestMethod12`）を優先
- 大量診断（CS0234 等）は想定内。適用可否は `applied` で判断し、実ファイルは `read_script` で確認
- テスト対象大ファイル: `Assets/Scripts/GigaTestFile.cs`
- `preview=true` は診断確認時のみ。適用時は `preview=false`

---

## S10) シンボル特定（get_script_symbols / find_script_symbol）

正常系:
- S10-01: `get_script_symbols` で `FinalTestClass` と `TestMethod11`、および `TestMethod12`（不在時は `LLMTEST_TestMethod12` 可）を確認（開始行・コンテナ含む）
- S10-02: 必要に応じ `find_script_symbol` で namePath を補助確定

異常系:
- S10-E01: `Assets/Nope.cs` で `get_script_symbols` → `fail`
- S10-E02: 極端に大きいファイルでの完了性確認（本APIは上限/ページ指定無し）
  - 期待: `get_script_symbols` が正常完了し、`symbols` が非空。完了時間が実用範囲（目安 ≤ 10s）。
...
## S20) 置換（edit_script_structured）整形検証

正常系:
- S20-01: `replace_body`（`symbolName=FinalTestClass/LLMTEST_ReturnInt`, `newText={ return 99; }`）→ `applied=true`。`read_script` の 30–40 行でインデント自然
...
判定条件・復元:
- 判定: 応答 `applied=true` かつ `read_script` 抜粋に `return 99;` が反映。インデント崩れがない。
- 復元: 実行前に `read_script` で本体スニペットを保存。終了時に `replace_body` で元本体に戻し、`beforeHash==afterHash` を確認。
...
## S30) リネーム（rename_script_symbol）＋参照確認

正常系:
- S30-01: `namePath=FinalTestClass/LLMTEST_RenameMe` → `newName=LLMTEST_RenameMe_Renamed` → `applied=true`
- S30-02: `read_script` で宣言が更新済み
- S30-03: `find_script_refs`（`name=TestMethod11`）でコード参照 0（文字列/コメント除外）
...
- S30-X01: using エイリアス終端一致
  - `rename_script_symbol(relative=Assets/Scripts/DiceController.cs, namePath=DiceController, newName=DiceControllerV3, preview=false)`
  - 期待: `using DC = <...>.DiceController;` のような別名が `...DiceControllerV3` に更新（終端一致時のみ）。
- S30-X02: ネスト型
  - `rename_script_symbol(relative=Assets/Scripts/GigaTestFile.cs, namePath=Outer/Inner, newName=InnerX, preview=false)`
  - 期待: `Outer` 直下の `Inner` のみ更新（他 `Inner` は未変更）。
- S30-X03: ジェネリック
  - 型: `rename_script_symbol(relative=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass, newName=FinalTestClass2, preview=false)`
  - メンバー: `rename_script_symbol(relative=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/GenericMethod, newName=GenericMethod2, preview=false)`
  - 期待: 型は横断更新、メンバーは宣言ファイル内のみ。
- S30-X04: オーバーロード
  - `rename_script_symbol(relative=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/Roll, newName=RollFast, preview=false)`（パラメータ無し版を対象）
  - 期待: 他の `Roll(int)` は未変更。
- S30-X05: プロパティ/イベント
  - `rename_script_symbol(relative=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/Score, newName=Score2, preview=false)`
  - `rename_script_symbol(relative=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/OnRolled, newName=OnRolled2, preview=false)`
  - 期待: 宣言ファイル内のみ更新。

判定条件・復元:
- 判定: `applied=true`、宣言が新名に更新、`find_script_refs(name=旧名)` のコード参照 0。
- 復元: `rename_script_symbol` で元名へ戻す（実行前に旧名を保存）。
  環境制約: 適用不可（dry-run/参照未解決など）で `applied=false` の場合は `skip（環境制約）` とし、details に `success/applied/errorCount` 等を記録。

---

## S40) シンボル削除（remove_script_symbol）

正常系:
- S40-01: `failOnReferences=false` で削除適用（対象: `FinalTestClass/LLMTEST_RemoveMe`。存在すれば `TestMethod12` を用いても可）→ `applied=true`、`read_script` で欠落確認
...
- S40-X01: 部分クラスの私有メソッド
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=PartialClass/LLMTEST_Helper, apply=true, failOnReferences=true)`
  - 期待: 参照が無ければ削除成功／あればブロック。
- S40-X02: 属性
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/LLMTEST_Attr, apply=true, failOnReferences=true)`
  - 期待: 削除適用（構文維持）。
- S40-X03: 明示実装
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=IFoo/Bar, apply=true, failOnReferences=true)`
  - 期待: 参照ありでブロック。
- S40-X04: イベント
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/OnRolled, apply=true, failOnReferences=true)`
  - 期待: 参照があればブロック、無ければ成功。
- S40-X05: enum メンバー
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=SomeEnum/MemberX, apply=true, failOnReferences=true)`
  - 期待: 未使用なら成功。

判定条件・復元:
- 判定: `applied=true` かつ `read_script` で該当メソッドが欠落。
- 復元: 実行前に「完全スニペット（宣言＋本体）」を保存し、`edit_script_structured` の `insert_after`（または `insert_before`）で `FinalTestClass` 内に再挿入。`beforeHash==afterHash` を確認。

実行手順（例示・パラメータ完全指定）:
- 事前取得: `read_script(path=Assets/Scripts/GigaTestFile.cs, startLine=対象周辺, endLine=対象周辺)` で対象スニペットを保存（宣言行含む）。
- S40-01 実行: `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/LLMTEST_RemoveMe, apply=true, failOnReferences=false, removeEmptyFile=false)`
  - 期待: `success:true, applied:true`。続けて `read_script` で対象行の欠落を確認。
  - 復元: `edit_script_structured(operation=insert_after, path=Assets/Scripts/GigaTestFile.cs, symbolName=FinalTestClass, newText="        private void LLMTEST_RemoveMe() { }\n")`
- S40-E01 実行: 参照が存在するシンボル例 `BaseProcessor/GetProcessorName` を対象
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=BaseProcessor/GetProcessorName, apply=true, failOnReferences=true)`
...
- S40-E02 実行: `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass/ThisDoesNotExist, apply=true, failOnReferences=false)`
  - 期待: `success:false` または `applied:false` とエラーメッセージ。
  - 観測不能時の扱いは E01 と同様に `skip（診断未返却）`。

---

## S50) 参照検索（find_script_refs）ページング/要約
...
- S50-X01: コメント/文字列の誤検出抑制
  - `find_script_refs(name=FinalTestClass, scope=assets, pageSize=50, maxBytes=65536, maxMatchesPerFile=5, snippetContext=2)`
...
実行手順（パラメータ完全指定）:
- S50-01: `find_script_refs(name=FinalTestClass, namespace=, container=, kind=, scope=assets, pageSize=20, maxBytes=65536, maxMatchesPerFile=3, snippetContext=1)`
  - 期待: `success:true`。対象が少ない場合は `truncated:false` でも可。ヒットが多い場合は `truncated:true` 且つ各 `snippetTruncated` が最大400文字トリムに従う。
- S50-E01: `find_script_refs(name=FinalTestClass, scope=assets, pageSize=1, maxBytes=1, maxMatchesPerFile=1, snippetContext=0)`
...
実行手順（テストドライバの例）:
- S60-01: `edit_script_structured(operation=replace_body, path=Assets/Scripts/GigaTestFile.cs, symbolName=FinalTestClass/TestMethod11, newText="{\n    this is not valid C#;\n}\n", preview=true)`
...
実行手順（具体例）:
- S70-01: パス正規化/拒否（安全側）
  - `get_script_symbols(path=UnityMCPServer/Assets/Scripts/GigaTestFile.cs)`（冗長/不正確パス）
  - 期待: 非破壊で安全側（正規化して成功 または 拒否）。どちらでも pass とし、応答の挙動を details に記録。
- S70-02: 曖昧 `namePath` の安全側動作（非破壊）
  - `edit_script_structured(operation=replace_body, path=Assets/Scripts/GigaTestFile.cs, symbolName=FinalTestClass/*, newText="{ return 0; }", preview=true)`
  - 期待: 適用されない（`applied:false`）か、複数一致のため拒否。コード変更が発生しないこと。
- S70-E01: 破壊的誤用の拒否
  - `remove_script_symbol(path=Assets/Scripts/GigaTestFile.cs, namePath=FinalTestClass, apply=false, failOnReferences=true)`
  - 期待: `success:false` または `applied:false` と妥当なエラーが返る。
  - 診断が返らない場合は `skip（プレビュー診断未返却）`。

---

## S80) 部分読み取り/サイズ制限（read_script）
...
## S82) 軽量スニペット編集（edit_script_snippet）
...
実行手順（例: nullガード削除）:
1. `read_script` で対象メソッドを取得し、削除対象のスニペットを正確にコピー
2. `edit_script_snippet` を以下のように呼び出す
   ```json
   {
