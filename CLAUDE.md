...
- **必須**: 以下のunity-mcp-serverツールを使用
  - `get_script_symbols`: ファイル内のシンボル（クラス、メソッド、フィールド等）を取得
  - `find_script_symbol`: シンボルを名前で検索
  - `find_script_refs`: シンボルの参照箇所を検索
  - `edit_script_structured`: 構造化編集（メソッド本体置換、クラスメンバー追加）
  - `edit_script_snippet`: 軽量スニペット編集（1〜2行、80文字以内）
  - `read_script`: C#ファイルの読み取り
  - `search_script`: C#コード内の検索
...
|------|------------------------|------------|------|
| シンボル検索 | `find_script_symbol` | `grep` | **コードインデックス勝利**（瞬時 vs 353ms） |
| 参照検索 | `find_script_refs` | `grep` | **コードインデックス勝利**（瞬時 vs 346ms） |
| コード検索 | `search_script` | `grep` | **コードインデックス勝利**（瞬時 vs 346ms） |
| ファイル読み取り | `read_script` | `Read` | **同等**（両方瞬時） |

**コンテキスト圧縮**:

| ツール | 出力サイズ | 標準ツール | 圧縮率 |
|--------|-----------|------------|--------|
| `read_script` | 200行 (8KB) | 358行 (13KB) | **1.6倍小さい** |
| `find_script_symbol` | 34シンボル (3KB) | 209行 (15KB) | **5倍小さい** |
| `find_script_refs` | 20参照 (4KB) | 全行 | **3倍小さい** |

**重要**: `build_code_index` を実行してDBインデックスを構築してから使用してください。インデックスなしではLSPタイムアウト（60秒）が発生します。

#### コードインデックスの活用

**編集前の必須ステップ**:

1. **`get_script_symbols`**: 編集対象ファイルのシンボル構造を把握
   - クラス、メソッド、フィールド、プロパティの一覧を取得
   - 各シンボルの位置（行・列）を確認
   - namePath（例: `Outer/Nested/Member`）を特定

2. **`find_script_symbol`**: シンボルを名前で検索
   - プロジェクト全体から特定のクラス/メソッドを検索
   - 部分一致検索も可能（`substring_matching=true`）
   - スコープ指定（Assets/Packages）で絞り込み

3. **`find_script_refs`**: 影響範囲の確認
...
1. **`edit_script_structured`**: 構造化編集
   - メソッド本体の完全置換
   - クラスメンバーの追加
   - namePath指定で対象を一意に特定

2. **`edit_script_snippet`**: 軽量スニペット編集
   - 1〜2行の小さな変更
   - アンカー文字列で編集位置を厳密に指定

**検証**:

1. **`get_code_index_status`**: インデックス状態の確認
   - カバレッジ率を確認
   - インデックスが古い場合は`update_code_index`で更新

**ワークフロー例**:

```
1. get_script_symbols → クラス構造を把握
2. find_script_refs → 影響範囲を確認
3. edit_script_structured → メソッド本体を置換
4. update_code_index → インデックスを更新
5. find_script_refs → 変更後の参照箇所を再確認
```
...
#### `edit_script_structured`: 構造化編集（メソッド本体、クラスメンバー追加）
...
3. **`diff exceeds 80 characters`**: 差分が80文字超
   - **解決策**: `edit_script_structured` に切り替え
...
**Q: アンカーが見つからない**
- A: `read_script` でファイル内容を確認し、空白・改行を含む正確な文字列をコピー

**Q: 複数箇所にマッチしてしまう**
- A: アンカーに前後数行のコンテキストを含めて一意にする

**Q: 80文字制限に引っかかる**
- A: `edit_script_structured` で replace_body を使用
...
**警告のみ（ブロックしない）**:
- `Read` ツールでのUnity C#ファイル読み取り（代わりに`mcp__unity-mcp-server__read_script`を推奨）
...
- `mcp__unity-mcp-server__search_script`
