...
## Unity MCP Server - Tool Discovery（LLM向け重要指示）

### ツール発見フロー（必須）

Unity MCP Serverは**intelligent tool loading**を実装しており、起動時にLLMに登録されるツールは`search_tools`のみです（96.2%トークン削減: ~97.2k → ~2k tokens）。

**重要**: 他の102個のツールを使用する前に、必ず以下の手順を実行してください：

1. **`search_tools`を最初に呼び出す**
   - キーワード、カテゴリ、タグ、スコープで検索可能
   - 関連度スコアリングで結果をソート

2. **必要なツールを特定**
   - `search_tools`の結果からツール名を取得
   - `includeSchemas: true`で詳細なinputSchemaも取得可能

3. **通常通りツールを実行**
   - ツール名を知っていれば`CallTool`で直接実行可能
   - 103個のツール全てが引き続き利用可能
...
// ステップ1: ツールを検索
{
  "tool": "search_tools",
  "params": {
    "query": "gameobject create",
    "limit": 5
  }
}
// 結果: gameobject_create, gameobject_find, gameobject_modify, ...

// ステップ2: 見つけたツールを実行
{
  "tool": "gameobject_create",
  "params": {
    "name": "TestCube",
    "primitiveType": "cube"
  }
...
- ❌ **`search_tools`をスキップしない**: 最初に必ず`search_tools`を呼び出してツールを発見
- ❌ **存在しないツールを推測しない**: 必ず`search_tools`で確認
- ❌ **古い知識に依存しない**: ListToolsは`search_tools`のみを返す（以前の103ツールは返さない）
