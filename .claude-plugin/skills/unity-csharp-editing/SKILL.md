...
```javascript
// インデックス状態を確認
mcp__unity-mcp-server__get_code_index_status()

// インデックスを構築（初回のみ、数分かかる場合あり）
mcp__unity-mcp-server__build_code_index()
```
...
| ツール | 目的 | 標準比 |
|--------|------|--------|
| `find_script_symbol` | シンボル検索 | 5倍小さい出力 |
| `find_script_refs` | 参照検索 | 3倍小さい出力 |
| `search_script` | コード検索 | 瞬時レスポンス |
...
```
変更が80文字以内？
├─ YES → edit_script_snippet
└─ NO → メソッド/プロパティ全体の置換？
         ├─ YES → edit_script_structured (replace_body)
         └─ NO → クラスへの追加？
                  ├─ YES → edit_script_structured (insert_after)
                  └─ NO → 複数のsnippetに分割
```
...
// 3. インデックス更新
mcp__unity-mcp-server__update_code_index({
...
mcp__unity-mcp-server__rename_script_symbol({
...
// 参照がなければ削除
mcp__unity-mcp-server__remove_script_symbol({
  path: "Assets/Scripts/Player.cs",
...
**解決策**: `read_script`でファイル内容を確認し、空白・改行を含む正確な文字列をコピー
...
| ツール | 用途 | 主要パラメータ |
|--------|------|----------------|
| `get_script_symbols` | ファイル内シンボル一覧 | path |
| `find_script_symbol` | シンボル検索 | name, kind, scope, exact |
| `find_script_refs` | 参照検索 | name, container, scope |
| `read_script` | コード読み取り | path, startLine, endLine |
| `search_script` | パターン検索 | pattern, patternType, scope |
| `edit_script_snippet` | 軽量編集（80文字以内） | path, instructions |
| `edit_script_structured` | 構造化編集 | path, symbolName, operation, newText |
| `create_script_class` | クラス作成 | path, className, namespace, baseType |
| `rename_script_symbol` | リネーム | relative, namePath, newName |
| `remove_script_symbol` | シンボル削除 | path, namePath, failOnReferences |
| `get_code_index_status` | インデックス状態確認 | - |
| `build_code_index` | インデックス構築 | - |
| `update_code_index` | インデックス更新 | paths |
